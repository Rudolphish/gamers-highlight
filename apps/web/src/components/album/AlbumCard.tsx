    ...
    <div className="group overflow-hidden rounded-sm border border-steam-border bg-steam-surface transition hover:border-steam-blue hover:shadow-[0_0_16px_-2px_rgba(102,192,244,0.5)]">
      <div className="relative h-32 w-full overflow-hidden bg-steam-panel">
        {coverImageUrl ? (
          ...
          <div className="flex h-full w-full items-center justify-center font-mono text-[11px] text-steam-muted/60">
            まだ投稿なし
          </div>
        )}
        {photoCount > 0 && (
          <div className="absolute right-1.5 top-1.5 rounded-sm bg-steam-bg/80 px-1.5 py-0.5 font-mono text-[10px] text-steam-blue">
            {photoCount}
          </div>
        )}
      </div>
