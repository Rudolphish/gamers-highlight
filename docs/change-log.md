...
> **この進め方は 2026-08-11 で終わっている（履歴として残しているだけ）。**
> Google AI Studio / VSCodeエージェントとの分担開発は行っておらず、いまは
> ブランチ＋PR単位で進めている（[`handoff.md`](./handoff.md) の「進め方の約束」）。
> ここに書かれたタスク（T1〜T13）は全て完了済み。**新しい作業をここに足さないこと。**
...
## [Phase 6] ゲーム詳細ページに実際のレビュー本文を表示

- 日時: 2026-08-08
- 担当ツール: Claude（直接実装）
- 変更ファイル:
  - `apps/web/src/lib/steam.ts`（変更：`getSteamReviews`〈レビュー本文取得〉、`stripSteamBBCode`〈BBCode/HTML除去の共通関数〉を追加）
  - `apps/web/src/app/(main)/groups/[groupId]/games/[gameId]/page.tsx`（変更：レビュー本文カードを追加。ニュース本文の整形処理も新しい共通関数に置き換え）
- 変更内容の要約:
  - これまでレビューは評価概要（「非常に好評」等＋件数）のみだったが、ユーザー要望で実際のレビュー本文も表示するように拡張。既存の`appreviews`エンドポイントが`reviews`配列に本文・👍👎・プレイ時間・投稿日時を含んでいることを実データで確認して利用（新しいAPIキーは不要）
  - 「希望言語のレビュー」という要望に対しては、既存のSteam連携と同じ`language=japanese`を指定（Steam側の言語判定は完全ではなく、実データ確認では稀に英語レビューも混ざることを確認したが、大部分は日本語になる）
  - レビュー本文にもニュースと同様BBCode/HTMLが混じるため、既存の個別関数だったニュース整形ロジックを`lib/steam.ts`の`stripSteamBBCode`として共通化し、両方から使うようにリファクタ
  - 表示は最新3件、各レビューを4行までに制限（`line-clamp-4`）。取得0件時はカード自体を非表示にする既存パターンを踏襲
- 完了条件チェック:
  - [x] ゲーム詳細ページにレビュー本文（👍/👎・プレイ時間・投稿日時付き）が最大3件表示される
  - [x] レビューが0件の場合はカードが表示されない
  - [x] BBCodeタグが本文に残らずプレーンテキストで表示される
- 実行したコマンド:
  - 実データ（Elden Ring）で`getSteamReviews`の動作確認、BBCode除去後のテキストを確認
  - `pnpm --filter web build`（成功）
- 懸念点・確認してほしいこと:
  - 実機でレビューカードの見た目を確認してほしい
  - Steamの言語フィルタは完全ではないため、まれに日本語以外のレビューが表示される可能性がある

## [区切り] Phase 6・7の新機能実装が一区切り、次は改善系タスクへ

- 日時: 2026-08-08
- 担当ツール: Claude（相談・整理のみ、実装は次回セッション）
- 内容の要約:
  - ユーザーから「新機能としては一通り希望のものを詰め込んだので一区切り。次からは新機能ではなく改善系」という方針転換の申し出があり、3点の課題を共有された：
    1. ページ遷移の整合性・遷移しやすいUI
    2. 別グループで同一ゲームを登録した時の挙動への懸念
    3. 状態管理（登録/削除操作の反映待ち）によるUXの分かりにくさ
  - その場で調査・回答した内容：
    - 1について：`grep`で実際に調査し、「戻る」リンクが`albums/new`・`groups/new`・`groups/[groupId]/albums/new`・ゲーム詳細ページには存在するが、**アルバム詳細ページ・グループ詳細ページには無い**ことを確認（サイドバー経由の移動のみ前提の作り）
    - 2について：`GroupGame`は`@@unique([groupId, steamAppId])`でグループごとに独立したレコードになるため、データの衝突・上書きは起きないと確認（正常動作）。ただしSteam/YouTube/HowLongToBeatへの外部問い合わせがグループごとに重複して走る非効率はある（バグではなく最適化余地）
    - 3について：現状のミューテーション系コンポーネントは「API呼び出し→成功後`router.refresh()`」というパターンで、往復中は画面が変わらないため無反応に見える、という診断で合意。楽観的更新（React `useOptimistic`等）の段階的導入が定石の対応
