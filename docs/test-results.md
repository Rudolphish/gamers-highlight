# テスト項目と結果

`tools/local-test/` の各スイートが出力した結果をそのまま表にしたもの。
**この表は手で書き換えない。** 項目を足すときはスクリプト側に足して、
`node tools/local-test/run-all.mjs` で作り直す（手順は
[`tools/local-test/README.md`](../tools/local-test/README.md)）。

最終実行: **2026/9/14 1:28:44**（JST） / 合計 **420** 件 / NG **0** 件

スイートは個別に流せるので、**実行時刻はスイートごとに違うことがある**
（変更に関係する箇所だけ流し直す使い方を想定している）。
実行時刻が古いスイートは、その時点のコードに対する結果でしかない。

| スイート | 内容 | 件数 | NG | 実行時刻(JST) |
|---|---|---:|---:|---|
| pages | P: ページの到達性と権限 | 95 | 0 | 2026/9/14 1:18:48 |
| api | R: APIの権限 | 62 | 0 | 2026/9/14 1:18:48 |
| flows | F: 主要導線 | 188 | 0 | 2026/9/14 1:18:54 |
| external-failure | X: 外部APIが落ちている状態 | 5 | 0 | 2026/9/14 1:18:55 |
| browser | B: 実ブラウザでの描画 | 70 | 0 | 2026/9/14 1:28:44 |

外部サービスはすべてスタブなので、**先方の仕様変更で壊れる類の不具合はここには出ない**。
実機でしか確認できないことは `docs/handoff.md` の「実機でしか確認できていないこと」にある。

---

## P: ページの到達性と権限

95 件 / NG 0 件 ・ 実行 2026/9/14 1:18:48

| ID | 項目 | 期待 | 実際 | 結果 | 備考 |
|---|---|---|---|---|---|
| P01/anon | ホーム（anon） | 307:/api/auth/signin | 307:/api/auth/signin | OK |  |
| P01/admin | ホーム（admin） | 200 | 200 | OK |  |
| P01/member | ホーム（member） | 200 | 200 | OK |  |
| P01/outsider | ホーム（outsider） | 200 | 200 | OK |  |
| P02/anon | ログイン画面（anon） | 200 | 200 | OK |  |
| P02/admin | ログイン画面（admin） | 200 | 200 | OK |  |
| P03/anon | アルバム一覧（anon） | 307:/api/auth/signin | 307:/api/auth/signin | OK |  |
| P03/admin | アルバム一覧（admin） | 200 | 200 | OK |  |
| P03/member | アルバム一覧（member） | 200 | 200 | OK |  |
| P03/outsider | アルバム一覧（outsider） | 200 | 200 | OK |  |
| P04/anon | アルバム作成（anon） | 307:/api/auth/signin | 307:/api/auth/signin | OK |  |
| P04/admin | アルバム作成（admin） | 200 | 200 | OK |  |
| P05/anon | 未分類の投稿（anon） | 307:/api/auth/signin | 307:/api/auth/signin | OK |  |
| P05/admin | 未分類の投稿（admin） | 200 | 200 | OK |  |
| P05/member | 未分類の投稿（member） | 200 | 200 | OK |  |
| P06/anon | アルバム詳細（自分が見られるもの）（anon） | 307:/api/auth/signin | 307:/api/auth/signin | OK |  |
| P06/admin | アルバム詳細（自分が見られるもの）（admin） | 200 | 200 | OK |  |
| P06/member | アルバム詳細（自分が見られるもの）（member） | 200 | 200 | OK |  |
| P06/outsider | アルバム詳細（自分が見られるもの）（outsider） | 見えない | 見えない | OK |  |
| P07/anon | アルバム詳細（他人のもの）（anon） | 307:/api/auth/signin | 307:/api/auth/signin | OK |  |
| P07/admin | アルバム詳細（他人のもの）（admin） | 見えない | 見えない | OK |  |
| P07/member | アルバム詳細（他人のもの）（member） | 見えない | 見えない | OK |  |
| P07/outsider | アルバム詳細（他人のもの）（outsider） | 200 | 200 | OK |  |
| P08/anon | グループ一覧（anon） | 307:/api/auth/signin | 307:/api/auth/signin | OK |  |
| P08/admin | グループ一覧（admin） | 200 | 200 | OK |  |
| P08/member | グループ一覧（member） | 200 | 200 | OK |  |
| P08/outsider | グループ一覧（outsider） | 200 | 200 | OK |  |
| P09/anon | グループ作成（anon） | 307:/api/auth/signin | 307:/api/auth/signin | OK |  |
| P09/admin | グループ作成（admin） | 200 | 200 | OK |  |
| P10/anon | グループ詳細（自分のグループ）（anon） | 307:/api/auth/signin | 307:/api/auth/signin | OK |  |
| P10/admin | グループ詳細（自分のグループ）（admin） | 200 | 200 | OK |  |
| P10/member | グループ詳細（自分のグループ）（member） | 200 | 200 | OK |  |
| P10/outsider | グループ詳細（自分のグループ）（outsider） | 見えない | 見えない | OK |  |
| P11/anon | グループ詳細（他人のグループ）（anon） | 307:/api/auth/signin | 307:/api/auth/signin | OK |  |
| P11/admin | グループ詳細（他人のグループ）（admin） | 見えない | 見えない | OK |  |
| P11/member | グループ詳細（他人のグループ）（member） | 見えない | 見えない | OK |  |
| P11/outsider | グループ詳細（他人のグループ）（outsider） | 200 | 200 | OK |  |
| P12/anon | グループ内アルバム作成（anon） | 307:/api/auth/signin | 307:/api/auth/signin | OK |  |
| P12/admin | グループ内アルバム作成（admin） | 200 | 200 | OK |  |
| P12/member | グループ内アルバム作成（member） | 200 | 200 | OK |  |
| P13/anon | ゲーム詳細（anon） | 307:/api/auth/signin | 307:/api/auth/signin | OK |  |
| P13/admin | ゲーム詳細（admin） | 200 | 200 | OK |  |
| P13/member | ゲーム詳細（member） | 200 | 200 | OK |  |
| P13/outsider | ゲーム詳細（outsider） | 見えない | 見えない | OK |  |
| P14/anon | 提案詳細（anon） | 307:/api/auth/signin | 307:/api/auth/signin | OK |  |
| P14/admin | 提案詳細（admin） | 200 | 200 | OK |  |
| P14/member | 提案詳細（member） | 200 | 200 | OK |  |
| P14/outsider | 提案詳細（outsider） | 見えない | 見えない | OK |  |
| P15/anon | マニュアル（anon） | 307:/api/auth/signin | 307:/api/auth/signin | OK |  |
| P15/admin | マニュアル（admin） | 200 | 200 | OK |  |
| P16/anon | 検索（anon） | 307:/api/auth/signin | 307:/api/auth/signin | OK |  |
| P16/admin | 検索（admin） | 200 | 200 | OK |  |
| P16/member | 検索（member） | 200 | 200 | OK |  |
| P17/anon | アップロード（anon） | 307:/api/auth/signin | 307:/api/auth/signin | OK |  |
| P17/admin | アップロード（admin） | 200 | 200 | OK |  |
| P17/member | アップロード（member） | 200 | 200 | OK |  |
| P18/anon | 設定・プロフィール（anon） | 307:/api/auth/signin | 307:/api/auth/signin | OK |  |
| P18/admin | 設定・プロフィール（admin） | 200 | 200 | OK |  |
| P18/member | 設定・プロフィール（member） | 200 | 200 | OK |  |
| P19/anon | 設定・許可リスト（anon） | 307:/api/auth/signin | 307:/api/auth/signin | OK |  |
| P19/admin | 設定・許可リスト（admin） | 200 | 200 | OK |  |
| P19/member | 設定・許可リスト（member） | 200 | 200 | OK |  |
| P20/anon | 設定・Discord連携（anon） | 307:/api/auth/signin | 307:/api/auth/signin | OK |  |
| P20/admin | 設定・Discord連携（admin） | 200 | 200 | OK |  |
| P21/anon | 設定・チャンネル対応（anon） | 307:/api/auth/signin | 307:/api/auth/signin | OK |  |
| P21/admin | 設定・チャンネル対応（admin） | 200 | 200 | OK |  |
| P22/anon | 管理・使用量（管理者のみ）（anon） | 307:/api/auth/signin | 307:/api/auth/signin | OK |  |
| P22/admin | 管理・使用量（管理者のみ）（admin） | 200 | 200 | OK |  |
| P22/member | 管理・使用量（管理者のみ）（member） | 見えない | 見えない | OK |  |
| P22/outsider | 管理・使用量（管理者のみ）（outsider） | 見えない | 見えない | OK |  |
| P23/anon | 管理・エラー（anon） | 307:/api/auth/signin | 307:/api/auth/signin | OK |  |
| P23/admin | 管理・エラー（admin） | 200 | 200 | OK |  |
| P23/member | 管理・エラー（member） | 見えない | 見えない | OK |  |
| P24/anon | 管理・招待リンク（anon） | 307:/api/auth/signin | 307:/api/auth/signin | OK |  |
| P24/admin | 管理・招待リンク（admin） | 200 | 200 | OK |  |
| P24/member | 管理・招待リンク（member） | 見えない | 見えない | OK |  |
| P25/anon | 管理・メディア一覧（anon） | 307:/api/auth/signin | 307:/api/auth/signin | OK |  |
| P25/admin | 管理・メディア一覧（admin） | 200 | 200 | OK |  |
| P25/member | 管理・メディア一覧（member） | 見えない | 見えない | OK |  |
| P26/anon | 管理・ユーザー（anon） | 307:/api/auth/signin | 307:/api/auth/signin | OK |  |
| P26/admin | 管理・ユーザー（admin） | 200 | 200 | OK |  |
| P26/member | 管理・ユーザー（member） | 見えない | 見えない | OK |  |
| P26b/anon | 管理・週次まとめ（anon） | 307:/api/auth/signin | 307:/api/auth/signin | OK |  |
| P26b/admin | 管理・週次まとめ（admin） | 200 | 200 | OK |  |
| P26b/member | 管理・週次まとめ（member） | 見えない | 見えない | OK |  |
| P26b/outsider | 管理・週次まとめ（outsider） | 見えない | 見えない | OK |  |
| P26c/anon | 管理・活動カレンダー（anon） | 307:/api/auth/signin | 307:/api/auth/signin | OK |  |
| P26c/admin | 管理・活動カレンダー（admin） | 200 | 200 | OK |  |
| P26c/member | 管理・活動カレンダー（member） | 見えない | 見えない | OK |  |
| P26c/outsider | 管理・活動カレンダー（outsider） | 見えない | 見えない | OK |  |
| P27/anon | 招待リンク（有効・未ログインでも開ける）（anon） | 200 | 200 | OK |  |
| P27/admin | 招待リンク（有効・未ログインでも開ける）（admin） | 200 | 200 | OK |  |
| P28/anon | 招待リンク（期限切れ）（anon） | 200 | 200 | OK |  |
| P29/anon | 招待リンク（取り消し済み）（anon） | 200 | 200 | OK |  |
| P30/anon | 招待リンク（存在しない）（anon） | 200 | 200 | OK |  |

---

## R: APIの権限

62 件 / NG 0 件 ・ 実行 2026/9/14 1:18:48

| ID | 項目 | 期待 | 実際 | 結果 | 備考 |
|---|---|---|---|---|---|
| R01/anon | アルバム取得（他人のアルバム）（anon） | 401 | 401 | OK |  |
| R01/admin | アルバム取得（他人のアルバム）（admin） | 403 | 403 | OK |  |
| R01/member | アルバム取得（他人のアルバム）（member） | 403 | 403 | OK |  |
| R01/outsider | アルバム取得（他人のアルバム）（outsider） | 200 | 200 | OK |  |
| R02/anon | アルバムの写真一覧（他人のアルバム）（anon） | 401 | 401 | OK |  |
| R02/admin | アルバムの写真一覧（他人のアルバム）（admin） | 403 | 403 | OK |  |
| R02/outsider | アルバムの写真一覧（他人のアルバム）（outsider） | 200 | 200 | OK |  |
| R03/admin | アルバム取得（自分が見られるもの）（admin） | 200 | 200 | OK |  |
| R03/member | アルバム取得（自分が見られるもの）（member） | 200 | 200 | OK |  |
| R03/outsider | アルバム取得（自分が見られるもの）（outsider） | 403 | 403 | OK |  |
| R04/outsider | アルバム削除（権限なし）（outsider） | 403 | 403 | OK |  |
| R05/outsider | アルバム更新（権限なし）（outsider） | 403 | 403 | OK |  |
| R06/outsider | 他人のグループにアルバム作成（outsider） | 403 | 403 | OK |  |
| R07/anon | グループ一覧（自分の所属だけ返る）（anon） | 401 | 401 | OK |  |
| R07/admin | グループ一覧（自分の所属だけ返る）（admin） | 200 | 200 | OK |  |
| R07/outsider | グループ一覧（自分の所属だけ返る）（outsider） | 200 | 200 | OK |  |
| R08/outsider | グループ取得（他人のグループ）（outsider） | 403 | 403 | OK |  |
| R09/outsider | グループのゲーム一覧（他人のグループ）（outsider） | 403 | 403 | OK |  |
| R10/outsider | 提案一覧（他人のグループ）（outsider） | 403 | 403 | OK |  |
| R11/outsider | 提案へのリアクション（他人のグループ）（outsider） | 403 | 403 | OK |  |
| R12/outsider | ゲームに「気になる」（他人のグループ）（outsider） | 403 | 403 | OK |  |
| R13/member | 招待リンク発行はOWNERのみ（member） | 403 | 403 | OK |  |
| R13/outsider | 招待リンク発行はOWNERのみ（outsider） | 403 | 403 | OK |  |
| R14/outsider | メンバー追加（権限なし）（outsider） | 403 | 403 | OK |  |
| R15/member | 写真削除（管理者は他人の写真も消せる＝仕様）（member） | 403 | 403 | OK |  |
| R15/admin | 写真削除（管理者は他人の写真も消せる＝仕様）（admin） | 200 | 200 | OK |  |
| R16/anon | 許可リスト一覧は管理者のみ（anon） | 403 | 403 | OK |  |
| R16/admin | 許可リスト一覧は管理者のみ（admin） | 200 | 200 | OK |  |
| R16/member | 許可リスト一覧は管理者のみ（member） | 403 | 403 | OK |  |
| R16/outsider | 許可リスト一覧は管理者のみ（outsider） | 403 | 403 | OK |  |
| R17/member | 許可リスト追加は管理者のみ（member） | 403 | 403 | OK |  |
| R17/outsider | 許可リスト追加は管理者のみ（outsider） | 403 | 403 | OK |  |
| R18/member | エラー通知先の変更は管理者のみ（member） | 403 | 403 | OK |  |
| R18/outsider | エラー通知先の変更は管理者のみ（outsider） | 403 | 403 | OK |  |
| R19/member | 招待の取り消し（管理者API）は管理者のみ（member） | 403 | 403 | OK |  |
| R19/outsider | 招待の取り消し（管理者API）は管理者のみ（outsider） | 403 | 403 | OK |  |
| R20/anon | cron（価格チェック）はシークレット必須（anon） | 401 | 401 | OK |  |
| R21/anon | 写真リアクションは閲覧できる人なら押せる（anon） | 401 | 401 | OK |  |
| R21/outsider | 写真リアクションは閲覧できる人なら押せる（outsider） | 403 | 403 | OK |  |
| R21/member | 写真リアクションは閲覧できる人なら押せる（member） | 200 | 200 | OK |  |
| R22/admin | 未分類の写真にはリアクションできない（admin） | 400 | 400 | OK |  |
| R23/anon | 写真の説明は見られる人なら書ける（anon） | 401 | 401 | OK |  |
| R23/outsider | 写真の説明は見られる人なら書ける（outsider） | 403 | 403 | OK |  |
| R23/member | 写真の説明は見られる人なら書ける（member） | 200 | 200 | OK |  |
| R24/admin | 未分類の写真には説明を付けられない（admin） | 400 | 400 | OK |  |
| R25/admin | 説明の文字数上限を超えると拒否（admin） | 400 | 400 | OK |  |
| R21/anon | cron（Bot死活）はシークレット必須（anon） | 401 | 401 | OK |  |
| R22/anon | 内部API（ゲーム一覧）はシークレット必須（anon） | 401 | 401 | OK |  |
| R23/anon | Discord取り込みはシークレット必須（anon） | 401 | 401 | OK |  |
| R24/anon | ゲーム判別（内部API）はシークレット必須（anon） | 401 | 401 | OK |  |
| R25/anon | Steam検索は要ログイン（anon） | 401 | 401 | OK |  |
| R25/admin | Steam検索は要ログイン（admin） | 200 | 200 | OK |  |
| R26/anon | 写真検索は要ログイン（anon） | 401 | 401 | OK |  |
| R26/admin | 写真検索は要ログイン（admin） | 200 | 200 | OK |  |
| R27/anon | ゲーム横断検索は要ログイン（anon） | 401 | 401 | OK |  |
| R27/admin | ゲーム横断検索は要ログイン（admin） | 200 | 200 | OK |  |
| R28/anon | ユーザー一覧は要ログイン（anon） | 401 | 401 | OK |  |
| R28/admin | ユーザー一覧は要ログイン（admin） | 200 | 200 | OK |  |
| R29/anon | 自分の情報は要ログイン（anon） | 401 | 401 | OK |  |
| R29/admin | 自分の情報は要ログイン（admin） | 200 | 200 | OK |  |
| R30/anon | アップロードURL発行は要ログイン（anon） | 401 | 401 | OK |  |
| R30/admin | アップロードURL発行は要ログイン（admin） | 201 | 201 | OK |  |

---

## F: 主要導線

188 件 / NG 0 件 ・ 実行 2026/9/14 1:18:54

| ID | 項目 | 期待 | 実際 | 結果 | 備考 |
|---|---|---|---|---|---|
| F01 | 署名付きURLが発行される | 成功 | 成功 | OK |  |
| F02 | 署名にチェックサムが混入していない（x-amz-checksum-） | 成功 | 成功 | OK |  |
| F03 | 署名付きPUTである（POSTポリシーではない） | 成功 | 成功 | OK |  |
| F04 | ストレージへの署名付きPUTが通る | 成功 | 成功 | OK |  |
| F05 | content-lengthが署名対象に含まれる（サイズ上限の担保） | 成功 | 成功 | OK |  |
| F06 | POSTは501（R2非対応の再現） | 成功 | 成功 | OK |  |
| F07 | アップロード後にPhotoが作られる | 成功 | 成功 | OK |  |
| F08 | 外部URLのmediaUrlは400で拒否 | 成功 | 成功 | OK |  |
| F09 | 権限の無いアルバムへの投稿は403 | 成功 | 成功 | OK |  |
| F10 | 共有アルバムへメンバーが投稿できる（#34） | 成功 | 成功 | OK |  |
| F11 | 長すぎる動画は413 | 成功 | 成功 | OK |  |
| F142 | 120秒ちょうどの動画は通る | 成功 | 成功 | OK |  |
| F143 | 121秒の動画は413 | 成功 | 成功 | OK |  |
| F144 | 100MBちょうどの動画は通る | 成功 | 成功 | OK |  |
| F145 | 100MBを1バイト超えた動画は413 | 成功 | 成功 | OK |  |
| F146 | 長さを送らない動画は通る（測れないファイルがあるため） | 成功 | 成功 | OK |  |
| F147 | 小数の長さは四捨五入して保存される（Prismaの切り捨てに任せない） | 成功 | 成功 | OK |  |
| F12 | OWNER以外は招待リンクを発行できない | 成功 | 成功 | OK |  |
| F13 | OWNERは招待リンクを発行できる | 成功 | 成功 | OK |  |
| F14 | claimが成功する | 成功 | 成功 | OK |  |
| F15 | 期限切れリンクのclaimは拒否 | 成功 | 成功 | OK |  |
| F16 | 取り消し済みリンクのclaimは拒否 | 成功 | 成功 | OK |  |
| F17 | acceptでグループに加入できる | 成功 | 成功 | OK |  |
| F18 | GroupMemberが作られている | 成功 | 成功 | OK |  |
| F19 | usedCountが増えている | 成功 | 成功 | OK |  |
| F20 | 使用履歴（GroupInviteUse）が残る | 成功 | 成功 | OK |  |
| F21 | 使用上限に達したリンクは拒否 | 成功 | 成功 | OK |  |
| F22 | リアクションを付けられる | 成功 | 成功 | OK |  |
| F23 | LIKEが過半数に達すると自動でACCEPTED＋GroupGame化 | 成功 | 成功 | OK |  |
| F24 | 昇格したゲームのカバーがappdetails由来（組み立てURLでない） | 成功 | 成功 | OK |  |
| F25 | 同じ人のリアクションは1件のまま | 成功 | 成功 | OK |  |
| F26 | Steam検索が結果を返す | 成功 | 成功 | OK |  |
| F27 | 検索結果からsub/bundleが除外されている | 成功 | 成功 | OK |  |
| F28 | ゲームを追加できる | 成功 | 成功 | OK |  |
| F29 | カバー画像がappdetailsのheader_image | 成功 | 成功 | OK |  |
| F30 | ジャンルが保存される | 成功 | 成功 | OK |  |
| F31 | YouTubeの動画IDが1回の検索で保存される | 成功 | 成功 | OK |  |
| F32 | HowLongToBeatの時間が保存される | 成功 | 成功 | OK |  |
| F33 | ExternalGameCacheに載る（2グループ目以降の再取得を防ぐ） | 成功 | 成功 | OK |  |
| F34 | YouTubeのクォータ消費が記録される | 成功 | 成功 | OK |  |
| F35 | 同じゲームの二重追加は弾かれる | 成功 | 成功 | OK |  |
| F36 | 「気になる」を付けられる | 成功 | 成功 | OK |  |
| F37 | シークレット無しの取り込みは401 | 成功 | 成功 | OK |  |
| F38 | Discordからの取り込みが成功する | 成功 | 成功 | OK |  |
| F39 | Photoが作られる（source=DISCORD） | 成功 | 成功 | OK |  |
| F40 | メディアが自前ストレージへコピーされている | 成功 | 成功 | OK |  |
| F41 | ハッシュタグからアルバムが自動作成される | 成功 | 成功 | OK |  |
| F42 | 同じメッセージの再取り込みで重複しない | 成功 | 成功 | OK |  |
| F43 | 判別APIが応答する | 成功 | 成功 | OK |  |
| F44 | app IDがゲーム名とアルバムに解決される | 成功 | 成功 | OK |  |
| F45 | 判別結果に既存アルバムが紐づく | 成功 | 成功 | OK |  |
| F46 | 未ログインからのエラー通報は401（通知の埋め立て対策） | 成功 | 成功 | OK |  |
| F47 | エラー通報を受け付ける | 成功 | 成功 | OK |  |
| F48 | ErrorReportに記録される | 成功 | 成功 | OK |  |
| F49 | 同じ不具合はcountに集約される | 成功 | 成功 | OK |  |
| F50 | cronはシークレット無しで401 | 成功 | 成功 | OK |  |
| F51 | cron（価格チェック）が完走する | 成功 | 成功 | OK |  |
| F52 | cron（Bot死活）が完走する | 成功 | 成功 | OK |  |
| F183 | Bot死活はグループの通知先には送らない（管理者側が未設定なら誰にも送らない） | 成功 | 成功 | OK |  |
| F184 | Bot死活は管理者の通知先へ1通だけ送る | 成功 | 成功 | OK |  |
| F53 | 外部が落ちていてもゲーム自体は追加できる | 成功 | 成功 | OK |  |
| F54 | 落ちている間は値が入らない（前提の確認） | 成功 | 成功 | OK |  |
| F55 | 復旧後は不足分だけ引き直して埋まる（失敗が焼き付かない） | 成功 | 成功 | OK |  |
| F56 | 埋まっているゲームは間隔内なら外部を引かない | 成功 | 成功 | OK |  |
| F57 | 未取得が残っていれば6時間で手動リフレッシュできる（429にならない） | 成功 | 成功 | OK |  |
| F57b | 短縮間隔のリフレッシュは埋まっているYouTubeを引き直さない | 成功 | 成功 | OK |  |
| F57c | 24時間経っていれば全ソースを引き直す | 成功 | 成功 | OK |  |
| F58 | 枠を半分使っていたら埋め直しのYouTube検索は行わない | 成功 | 成功 | OK |  |
| F59 | 旧トークン（userIdなし）でもページが見える | 成功 | 成功 | OK |  |
| F60 | 旧トークンでもAPIが通る | 成功 | 成功 | OK |  |
| F61 | 旧トークンでも権限判定は効く（他人のアルバムは403） | 成功 | 成功 | OK |  |
| F62 | アルバム詳細が開ける（キャッシュ作成） | 成功 | 成功 | OK |  |
| F62b | アルバム詳細はキャッシュヒット時も壊れない | 成功 | 成功 | OK |  |
| F63 | キャッシュ済みでも権限の無い人には出ない | 成功 | 成功 | OK |  |
| F64 | グループ詳細が開ける（キャッシュ作成） | 成功 | 成功 | OK |  |
| F64b | グループ詳細はキャッシュヒット時も壊れない | 成功 | 成功 | OK |  |
| F65 | グループもキャッシュ済みで権限の無い人には出ない | 成功 | 成功 | OK |  |
| F66 | 写真を上げたらアルバム詳細に即出る（無効化） | 成功 | 成功 | OK |  |
| F67 | 写真を消したらアルバム詳細から即消える（無効化） | 成功 | 成功 | OK |  |
| F68 | アルバム改名がグループ詳細にも即反映される（無効化） | 成功 | 成功 | OK |  |
| F69 | ゲーム追加がグループ詳細に即出る（無効化） | 成功 | 成功 | OK |  |
| F70 | 候補APIが応答する | 成功 | 成功 | OK |  |
| F71 | プレイ中のゲームが候補に出る | 成功 | 成功 | OK |  |
| F72 | プレイ中以外（気になる・積みゲー・クリア済み）は出ない | 成功 | 成功 | OK |  |
| F73 | 候補は24件で頭打ちになる（Discordの25個制限） | 成功 | 成功 | OK |  |
| F74 | ゲームを提案できる | 成功 | 成功 | OK |  |
| F75 | 提案の作成でクリア時間と関連動画がキャッシュに入る | 成功 | 成功 | OK |  |
| F76 | 提案のカバーがappdetails由来（組み立てURLでない） | 成功 | 成功 | OK |  |
| F77 | 提案ページにクリア時間と関連動画が出る | 成功 | 成功 | OK |  |
| F78 | 写真に❤️を付けられる | 成功 | 成功 | OK |  |
| F79 | もう一度押すと取り消される（トグル） | 成功 | 成功 | OK |  |
| F80 | 別のメンバーも押せて、1人1回に収まる | 成功 | 成功 | OK |  |
| F81 | アルバムページに❤️の数が出る | 成功 | 成功 | OK |  |
| F82 | リアクションのAPIは写真のキャッシュを飛ばさない | 成功 | 成功 | OK |  |
| F83 | 写真を消すとリアクションも消える | 成功 | 成功 | OK |  |
| F84 | 写真に説明を書ける | 成功 | 成功 | OK |  |
| F85 | 書き手と日時が残る | 成功 | 成功 | OK |  |
| F86 | 別のメンバーが上書きでき、書き手も入れ替わる | 成功 | 成功 | OK |  |
| F87 | 空にすると説明も書き手の記録も消える（写真は残る） | 成功 | 成功 | OK |  |
| F88 | 説明の文字列で写真を検索できる | 成功 | 成功 | OK |  |
| F89 | ゲームタイトルでの検索は従来どおり効く | 成功 | 成功 | OK |  |
| F90 | 説明を書き換えるとアルバムページに反映される（キャッシュが飛ぶ） | 成功 | 成功 | OK |  |
| F91 | ❤️を押すと活動ログに残り、groupIdが入る | 成功 | 成功 | OK |  |
| F92 | 取り消しも残る（付けた記録が消えない） | 成功 | 成功 | OK |  |
| F93 | 説明の「書いた」「消した」が別々に残る | 成功 | 成功 | OK |  |
| F94 | ステータス変更が from→to つきで残る（今週クリアしたゲームが出せる） | 成功 | 成功 | OK |  |
| F95 | 同じステータスで保存し直しても記録が増えない | 成功 | 成功 | OK |  |
| F96 | 写真の occurredAt は撮影日時（記録時刻ではない） | 成功 | 成功 | OK |  |
| F97 | 未分類をアルバムへ振り分けると groupId が埋まる（行は増えない） | 成功 | 成功 | OK |  |
| F99 | 同じ人を追加し直しても「加入」は1回しか記録されない | 成功 | 成功 | OK |  |
| F98 | 読むだけでは活動ログが増えない | 成功 | 成功 | OK |  |
| F100 | 週次まとめのページが管理者に出る | 成功 | 成功 | OK |  |
| F101 | 月曜0時（JST）ちょうどの記録は今週に数えられる | 成功 | 成功 | OK |  |
| F102 | その1ミリ秒前は先週に入る（今週は増えない） | 成功 | 成功 | OK |  |
| F103 | 今週クリアしたゲームが文面に出る | 成功 | 成功 | OK |  |
| F104 | クリア以外のステータス変更は「クリア」に出ない | 成功 | 成功 | OK |  |
| F105 | cronが完走する（ロールアップ込み） | 成功 | 成功 | OK |  |
| F106 | ロールアップは occurredAt の日に積む（記録した日ではない） | 成功 | 成功 | OK |  |
| F107 | cronを2回流しても件数が倍にならない（置き換え） | 成功 | 成功 | OK |  |
| F108 | 1年より古い生ログは消え、その日の件数は残る | 成功 | 成功 | OK |  |
| F109 | 保持期間内の生ログは消えない | 成功 | 成功 | OK |  |
| F110 | 消えたグループの集計は片付けられる | 成功 | 成功 | OK |  |
| F111 | 通知先が未設定なら送らず、送信済みにもしない | 成功 | 成功 | OK |  |
| F112 | 週次まとめの通知先を保存できる | 成功 | 成功 | OK |  |
| F113 | 通知先の保存は管理者以外は403 | 成功 | 成功 | OK |  |
| F114 | 未送信の完了週があれば、曜日に関係なくその週を対象にする | 成功 | 成功 | OK |  |
| F181 | 送信済みの記録がグループごとに付く | 成功 | 成功 | OK |  |
| F182 | 自動送信は全体の記録を進めない（グループごとの記録だけを使う） | 成功 | 成功 | OK |  |
| F115 | 送信済みの週は二度送らない（毎日のcronで毎日届かない） | 成功 | 成功 | OK |  |
| F116 | 手動送信は届くが、送信済みの記録は動かさない | 成功 | 成功 | OK |  |
| F117 | 送った文面が管理画面のプレビューと同じ形になっている | 成功 | 成功 | OK |  |
| F118 | 手動送信は管理者以外は403 | 成功 | 成功 | OK |  |
| F119 | 投稿に失敗したら「送った」に数えず、失敗として返す | 成功 | 成功 | OK |  |
| F120 | 活動カレンダーが管理者に出る | 成功 | 成功 | OK |  |
| F121 | 管理者以外には活動カレンダーの中身が出ない | 成功 | 成功 | OK |  |
| F122 | カレンダーは occurredAt の日に置く（記録した日ではない） | 成功 | 成功 | OK |  |
| F123 | JSTの0時ちょうどはその日に入る | 成功 | 成功 | OK |  |
| F124 | その1ミリ秒前は前日に入る（当日は増えない） | 成功 | 成功 | OK |  |
| F125 | 今日ぶんはロールアップを待たずに出る（生ログから数える） | 成功 | 成功 | OK |  |
| F126 | 1年より古い日はロールアップから件数だけ出す | 成功 | 成功 | OK |  |
| F127 | 消えた対象は名前だけ出し、リンクにはしない | 成功 | 成功 | OK |  |
| F128 | 他のグループの記録は混ざらない | 成功 | 成功 | OK |  |
| F129 | 手動アップロードでアルバムの updatedAt が進む | 成功 | 成功 | OK |  |
| F130 | Discord取り込みでもアルバムの updatedAt が進む | 成功 | 成功 | OK |  |
| F131 | 写真を削除しても updatedAt は進まない | 成功 | 成功 | OK |  |
| F132 | 説明を書いても updatedAt は進まない | 成功 | 成功 | OK |  |
| F133 | 投稿したアルバムがグループ画面で更新順の先頭に来る | 成功 | 成功 | OK |  |
| F134 | 前回の生存報告が無ければ遡らない | 成功 | 成功 | OK |  |
| F135 | 落ちていた間の投稿が遡って取り込まれる | 成功 | 成功 | OK |  |
| F136 | 同じメッセージを二度遡っても重複しない | 成功 | 成功 | OK |  |
| F137 | Botの投稿・添付なし・非対応形式・大きすぎる動画は取り込まない | 成功 | 成功 | OK |  |
| F138 | 長く落ちていても遡りは7日で打ち切る | 成功 | 成功 | OK |  |
| F139 | 生存報告は更新前の時刻を返す（遡りの起点になる） | 成功 | 成功 | OK |  |
| F140 | アルバム一覧はグループも返す（名前だけで選ばせないため） | 成功 | 成功 | OK |  |
| F141 | ファイル名からの判別結果にもグループ名が付く | 成功 | 成功 | OK |  |
| F148 | テスト用アルバムを作れる | 成功 | 成功 | OK |  |
| F149 | オーナーはアルバム名を変えられる | 成功 | 成功 | OK |  |
| F150 | 名前だけ変えても説明とサムネイルは消えない | 成功 | 成功 | OK |  |
| F151 | 空のアルバム名は400（名前は消えない） | 成功 | 成功 | OK |  |
| F152 | 100文字を超えるアルバム名は400 | 成功 | 成功 | OK |  |
| F153 | グループの編集者というだけでは他人のアルバム名を変えられない | 成功 | 成功 | OK |  |
| F154 | アルバムの編集者はアルバム名を変えられる | 成功 | 成功 | OK |  |
| F155 | 変えた名前がグループ画面に出る（キャッシュが残らない） | 成功 | 成功 | OK |  |
| F156 | YouTubeのURLを投稿できる（余計なパラメータは落として保存する） | 成功 | 成功 | OK |  |
| F157 | サムネイルは動画IDから組み立てられ、容量と長さは持たない | 成功 | 成功 | OK |  |
| F158 | youtu.be と shorts のURLも同じ正規形になる | 成功 | 成功 | OK |  |
| F159 | YouTube以外のURLは400（https://example.com/watch?v=dQw4w9WgXcQ） | 成功 | 成功 | OK |  |
| F160 | YouTube以外のURLは400（https://www.youtube.com/watch?v=short） | 成功 | 成功 | OK |  |
| F161 | YouTube以外のURLは400（javascript:alert(1)） | 成功 | 成功 | OK |  |
| F162 | YouTubeにはサイズ・長さの上限がかからない | 成功 | 成功 | OK |  |
| F163 | 権限の無いアルバムへのYouTube投稿は403 | 成功 | 成功 | OK |  |
| F164 | いちばん新しい投稿がYouTubeでも、アルバムのカバーには使われない | 成功 | 成功 | OK |  |
| F165 | APIが返すカバー候補にもYouTubeは混ざらない | 成功 | 成功 | OK |  |
| F166 | YOUTUBE として保存されている | 成功 | 成功 | OK |  |
| F167 | 提案するとDiscordへ通知が飛ぶ | 成功 | 成功 | OK |  |
| F168 | 通知にゲーム名と提案者が入っている | 成功 | 成功 | OK |  |
| F169 | 通知にメールアドレスが混ざらない | 成功 | 成功 | OK |  |
| F170 | 通知に必要な「いいね」の数が入っている（投票APIと同じ式） | 成功 | 成功 | OK |  |
| F171 | 通知先が未設定のグループでは送らない（提案自体は成功する） | 成功 | 成功 | OK |  |
| F172 | Discordが落ちていても提案は成功する | 成功 | 成功 | OK |  |
| F173 | 種類ごとに独立している（提案をオンにしても他の種類は空のまま） | 成功 | 成功 | OK |  |
| F174 | 種類ごとに別のチャンネルを指定できる | 成功 | 成功 | OK |  |
| F175 | 「送らない」に戻すと設定が消える | 成功 | 成功 | OK |  |
| F176 | 未設定のまま「送らない」を選んでも成功する | 成功 | 成功 | OK |  |
| F177 | チャンネルIDの形が違えば400 | 成功 | 成功 | OK |  |
| F178 | オーナー以外は通知先を変えられない | 成功 | 成功 | OK |  |
| F180 | 価格の通知先しか設定していなければ、提案の通知は飛ばない | 成功 | 成功 | OK |  |
| F179 | グループを消すと通知の設定も消える | 成功 | 成功 | OK |  |

---

## X: 外部APIが落ちている状態

5 件 / NG 0 件 ・ 実行 2026/9/14 1:18:55

| ID | 項目 | 期待 | 実際 | 結果 | 備考 |
|---|---|---|---|---|---|
| X01 | 外部API全滅時でも描画される: ホーム | 200 | 200 | OK |  |
| X02 | 外部API全滅時でも描画される: グループ詳細 | 200 | 200 | OK |  |
| X03 | 外部API全滅時でも描画される: ゲーム詳細（Steam・ITAD・YouTubeを全部引く） | 200 | 200 | OK |  |
| X04 | 外部API全滅時でも描画される: アルバム一覧（Steamのカバーを引く） | 200 | 200 | OK |  |
| X05 | 外部API全滅時でも描画される: 管理・使用量（R2とDBを実測する） | 200 | 200 | OK |  |

---

## B: 実ブラウザでの描画

70 件 / NG 0 件 ・ 実行 2026/9/14 1:28:44

| ID | 項目 | 期待 | 実際 | 結果 | 備考 |
|---|---|---|---|---|---|
| B01 | ブラウザで開いて例外が出ない: ホーム | 200 / 例外なし | 200 / 0件 | OK |  |
| B02 | ブラウザで開いて例外が出ない: アルバム一覧 | 200 / 例外なし | 200 / 0件 | OK |  |
| B03 | ブラウザで開いて例外が出ない: アルバム詳細 | 200 / 例外なし | 200 / 0件 | OK |  |
| B04 | ブラウザで開いて例外が出ない: 未分類の投稿 | 200 / 例外なし | 200 / 0件 | OK |  |
| B05 | ブラウザで開いて例外が出ない: グループ一覧 | 200 / 例外なし | 200 / 0件 | OK |  |
| B06 | ブラウザで開いて例外が出ない: グループ詳細 | 200 / 例外なし | 200 / 0件 | OK |  |
| B07 | ブラウザで開いて例外が出ない: ゲーム詳細 | 200 / 例外なし | 200 / 0件 | OK |  |
| B08 | ブラウザで開いて例外が出ない: 提案詳細 | 200 / 例外なし | 200 / 0件 | OK |  |
| B09 | ブラウザで開いて例外が出ない: アップロード | 200 / 例外なし | 200 / 0件 | OK |  |
| B10 | ブラウザで開いて例外が出ない: 検索 | 200 / 例外なし | 200 / 0件 | OK |  |
| B11 | ブラウザで開いて例外が出ない: マニュアル | 200 / 例外なし | 200 / 0件 | OK |  |
| B12 | ブラウザで開いて例外が出ない: 設定・プロフィール | 200 / 例外なし | 200 / 0件 | OK |  |
| B13 | ブラウザで開いて例外が出ない: 設定・許可リスト | 200 / 例外なし | 200 / 0件 | OK |  |
| B14 | ブラウザで開いて例外が出ない: 設定・Discord連携 | 200 / 例外なし | 200 / 0件 | OK |  |
| B15 | ブラウザで開いて例外が出ない: 管理・使用量 | 200 / 例外なし | 200 / 0件 | OK |  |
| B16 | ブラウザで開いて例外が出ない: 管理・ユーザー | 200 / 例外なし | 200 / 0件 | OK |  |
| B17 | ブラウザで開いて例外が出ない: 管理・招待リンク | 200 / 例外なし | 200 / 0件 | OK |  |
| B18 | ブラウザで開いて例外が出ない: 管理・メディア一覧 | 200 / 例外なし | 200 / 0件 | OK |  |
| B19 | ブラウザで開いて例外が出ない: 管理・エラー | 200 / 例外なし | 200 / 0件 | OK |  |
| B19b | ブラウザで開いて例外が出ない: 管理・活動カレンダー | 200 / 例外なし | 200 / 0件 | OK |  |
| B20 | ゲーム一覧の初期フィルタが「プレイ中」「気になる」だけ | プレイ中・気になるが選択済み | 更新順,プレイ中,気になる,全ジャンル | OK |  |
| B21 | 初期状態で積みゲー・クリア済みは出ない | 出ない | 出ていない | OK | ウィッチャー3 DOOM（積みゲ Dota 2管理者ユーザーが追加アクションRPG サイバーパンク2077管理者ユーザーが追加アクションRPG Grand Theft Auto V一般メンバーが追加アクションRPG No Imageウィッチ |
| B22 | 「すべて」を押すと積みゲー・クリア済みも出る | 出る | 出た | OK | ウィッチャー3 DOOM（積みゲ Dota 2管理者ユーザーが追加アクションRPG サイバーパンク2077管理者ユーザーが追加アクションRPG Grand Theft Auto V一般メンバーが追加アクションRPG No ImageHalf |
| B23 | 絞り込みがグループ単位のキーで保存される | 保存される | {"status":[],"genres":[]} | OK |  |
| B24 | リロードしても前回の絞り込みが復元される | 積みゲーも出る | 出た | OK |  |
| B25 | 保存を消すと既定（プレイ中・気になる）に戻る | 積みゲーは出ない | 出ていない | OK |  |
| B26 | 復元でハイドレーションのずれが出ない | 0件 | 0件 | OK |  |
| B27 | アルバムの初期の並びは更新順 | エルデンリング → ゼルダの伝説 → あつまれ どうぶつの森 | エルデンリング → ゼルダの伝説 → あつまれ どうぶつの森 | OK |  |
| B28 | 「更新順」が選択済みとして表示される | 更新順 | 更新順 | OK |  |
| B29 | 「新着順」は作成日時で並ぶ（更新順とは別の並び） | ゼルダの伝説 → あつまれ どうぶつの森 → エルデンリング | ゼルダの伝説 → あつまれ どうぶつの森 → エルデンリング | OK |  |
| B30 | 「名前順」は日本語の読みで並ぶ（あ→エ→ゼ） | あつまれ どうぶつの森 → エルデンリング → ゼルダの伝説 | あつまれ どうぶつの森 → エルデンリング → ゼルダの伝説 | OK |  |
| B31 | 「写真の多い順」は枚数が減る順に並ぶ | 降順 | 5 → 3 → 2 → 0 | OK |  |
| B31b | 「写真の多い順」で0枚のアルバムが3枚のアルバムより後ろに来る | ゼルダの伝説 が あつまれ どうぶつの森 より前 | エルデンリング → ゼルダの伝説 → あつまれ どうぶつの森 | OK |  |
| B32 | 「更新順」に戻せる | エルデンリング → ゼルダの伝説 → あつまれ どうぶつの森 | エルデンリング → ゼルダの伝説 → あつまれ どうぶつの森 | OK |  |
| B33 | グリッドの各写真に❤️ボタンが出る | 1つ以上 | 5個 | OK |  |
| B34 | ❤️を押すと押した状態になり件数が出る | 押した状態 / 1 | 押した状態=1 / 表示=1 | OK |  |
| B35 | もう一度押すと取り消せる | 0 | 0 | OK |  |
| B36 | Lightboxに❤️と押した人の名前が出る | ボタンあり / 管理者ユーザー | ボタン=1個 / 名前=true | OK | 1管理者ユーザー説明はまだありません 説明を書く |
| B37 | 次の写真へ移ると❤️の状態が引き継がれない | 0（この写真にはまだ付いていない） | 0 | OK |  |
| B38 | 説明が無い写真は「まだありません」と出て、書くボタンがある | 両方あり | 文言=true / ボタン=1 | OK |  |
| B39 | 説明を書いて保存すると、本文と書き手が出る | 本文 / 管理者ユーザー | 本文=true / 書き手=true | OK | 夜のリムグレイブ。月が写り込んでいる管理者ユーザー・2026/9/13 編集 |
| B40 | 説明を書くとグリッドに印が出る（再読み込み不要） | 1つ以上 | 2個 | OK |  |
| B41 | 再読み込みしても書いた説明が残る（キャッシュが飛んでいる） | 本文あり | 残っている | OK | 夜のリムグレイブ。月が写り込んでいる管理者ユーザー・2026/9/13 編集 |
| B42 | 次の写真へ移ると前の写真の説明が残らない | 残らない | 残っていない | OK |  |
| B45 | グループを選ぶまでアルバムを選べない | 無効 | 無効 | OK |  |
| B46 | 選んだグループのアルバムだけが出る（同名でも混ざらない） | Aには片方だけ / Bには片方だけ | A=true B=true | OK |  |
| B47 | グループを切り替えるとアルバムの選択が外れる | 未選択 | 未選択 | OK |  |
| B48 | 新しいグループを作るリンクが出ている | 1件以上 | 1 | OK |  |
| B49 | マニュアルの権限の節に表が出る | できること／必要な権限の行がある | 出ている | OK |  |
| B50 | マニュアルの未分類の節が開く | 本文が出る | 出ている | OK |  |
| B51 | アルバムのオーナーには名前の変更ボタンが出る | 出る | 出ている | OK |  |
| B52 | 名前を書き換えると見出しが変わる | エルデンリング（改名テスト） | エルデンリング（改名テスト） | OK |  |
| B53 | 空のアルバム名は保存できない | その場でエラーが出る | 出た | OK |  |
| B54 | テストの後始末で元の名前に戻っている | エルデンリング | エルデンリング | OK |  |
| B55 | アルバムに招待されていない人には変更ボタンが出ない（閲覧はできる） | ボタン無し／中身は見える | ボタン0件 / 見出しあり | OK |  |
| B56 | YouTubeの投稿がアルバムに並び、種類が分かる印が出る | 印が出る | 出ている | OK |  |
| B57 | 開くとYouTubeの埋め込み（nocookieドメイン）になる | https://www.youtube-nocookie.com/embed/bbbbbbbbbbb | https://www.youtube-nocookie.com/embed/bbbbbbbbbbb | OK |  |
| B58 | YouTubeの投稿を<video>で再生しようとしない | video要素が無い | 無い | OK |  |
| B59 | 通知の設定に3種類が並ぶ | 3種類とも出る | 3/3 | OK |  |
| B60 | プルダウンにDiscordのチャンネル一覧が出る（ボイスチャンネルは除く） | #general が出て #voice-chat は出ない | 送らない / #general / #screenshots / #sale-info | OK |  |
| B61 | 選んだ通知先が保存され、再読み込みしても残る | 900000000000000012 / 1種類がオン | 900000000000000012 / 通知の設定（1/3 種類がオン） | OK |  |
| B62 | 「送らない」に戻せる（後片付け） | 0種類がオン | 通知の設定（0/3 種類がオン） | OK |  |
| B63 | 手動アップロードでファイルが実際にアルバムへ入る | 2枚増える | 2枚 / 完了表示=あり | OK |  |
| B64 | 保存されたURLの先に実物が載っている | どれも 200 で中身が1バイト以上 | 200:70 / 200:70 | OK |  |
| B65 | アップロード中に4xx/5xxも例外も出ない | 無し | 無し | OK |  |
| B66 | 閲覧だけの人にも「追加」が出る（投稿はVIEWERで通るため） | 1件 | 1件 | OK |  |
| B67 | モーダルから複数ファイルがこのアルバムへ入る | 2枚増える | 2枚 / 完了表示=あり | OK |  |
| B68 | 再読み込みなしでグリッドに出る | 5枚 | 5枚 | OK |  |
| B69 | 同じモーダルからYouTubeのURLも足せる | 1件 / 正規形で保存 | 1件 / https://www.youtube.com/watch?v=ccccccccccc | OK |  |
| B70 | モーダルからの追加中に4xx/5xxも例外も出ない | 無し | 無し | OK |  |
