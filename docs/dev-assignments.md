...
本プロジェクトでは2つのAIツールを併用している：

- **Claude**（このチャット）：設計判断、複数ファイル横断の原因調査、ドキュメント化、**Google AI Studioの成果物レビュー**
- **Google AI Studio**：[`tasks.md`](./tasks.md)に定義された、小さく分割されたタスクを順番に実行

### レビューの回し方

1. Google AI Studioが[`tasks.md`](./tasks.md)のタスクを実装し、完了条件の自己チェックを行った上で[`change-log.md`](./change-log.md)に記録する
...
検索・ライトボックス・設定ページなど、Google AI Studioに割り当てている現在進行中のタスクはすべて[`tasks.md`](./tasks.md)（T1〜T10）に集約している。進捗確認はそちらを参照すること。
