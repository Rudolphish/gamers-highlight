...
**まだ実装していない。** これから作るものの設計と、**なぜその形にしたか**を残す。
実装が入ったらこのファイルを現状に合わせて直すこと。
...
| `proposal.accepted` / `proposal.rejected` / `proposal.withdrawn` | その時刻 | — |
...
遡り投入は**何度流しても同じ結果になるように**書くこと（途中で失敗して流し直すため）。
`kind + targetId` で既存を見て、無ければ入れる。
