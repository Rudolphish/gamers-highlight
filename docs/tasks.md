...
10. **タスクとして明示的に割り当てられていないファイルには絶対に手を出さない。** 特に`apps/web/src/lib/auth.ts`、`middleware.ts`、`.npmrc`、ルート`package.json`は**2回連続で重大な事故**（認証バイパスの混入、Vercelビルド設定の破壊）が起きている。「ついでに修正した方が良さそう」と思っても、タスク定義にない限り絶対に編集しない。気になる点があれば、変更せずに`change-log.md`に提案として書くだけにすること
