...
Vercelにも本番にも一切影響しない。外部サービスをスタブするので、
...
- `checks` … 型チェックと `audit-invalidation.mjs` / `audit-activity-log.mjs` / `audit-media-limits.mjs` / `audit-cover-policy.mjs`。サーバー不要なので数分で終わる
...
node tools/local-test/audit-cover-policy.mjs # アルバムカバー方針の静的チェック（サーバー不要）
```
...
**Bだけを単体で流すと B41・B42 が落ちる。** 説明まわりのテストが F の作ったデータに
依存しているため（`run-all.mjs` で流せば通る。CIも通しで流している）。実測で、
シードし直した直後にBだけを流すと再現し、通しで流すと通る。**Bで落ちたときは、
まず通しで流し直して切り分けること。**
...
| `audit-cover-policy.mjs` | アルバムのカバー候補を引くクエリが `lib/albumCoverPolicy.ts` を通っているかを静的に見る（サーバー不要）。「カバーではない」箇所は印を書いて宣言する |
